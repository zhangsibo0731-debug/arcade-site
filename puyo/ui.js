(function (global) {
  ...
  function create(options) {
    ...
    function renderChallenge(view) {
      ...
      elements.buildSummary.hidden = !active.length;
      elements.buildCount.textContent = active.length;
      elements.buildChips.innerHTML = active.map((item) => '<span data-upgrade="' + item.id + '" class="' + (view.triggeredUpgrades.has(item.id) ? 'is-triggered' : '') + '">' + item.name + ['','Ⅰ','Ⅱ','Ⅲ'][view.runBuild.upgrades[item.id]] + '</span>').join(' · ');
    }

    function renderUpgradeChoices(runBuild) {
      elements.upgradeKicker.textContent = runBuild.pendingKind === 'special' ? 'SPECIAL REWARD · 稀有保底' : 'STAGE REWARD';
    ...
    function renderBuildDetails(runBuild) {
      ...
      elements.buildDetails.innerHTML = active.map((item) => {
    ...
    function bindActions(actions) {
      ...
      elements.upgradeChoices.addEventListener('click', (event) => {
        ...
        if (button) actions.chooseUpgrade(button.dataset.upgrade);
      });
    }

    return Object.freeze({ selectGameType, renderHud, renderChallenge, renderUpgradeChoices, renderBuildDetails, showOverlay, showChallengeMessage, showChain, showChainResult, showLevel, flashGarbageDefense, resetTransient, bindActions });
  }
